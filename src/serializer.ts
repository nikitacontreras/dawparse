...
class BufferWriter {
  ...
  writeInt16(val: number) {
    ...
    const view = new DataView(this.buffer.buffer, this.buffer.byteOffset, this.buffer.byteLength);
    view.setInt16(this.offset, val, true);
    this.offset += 2;
  }
  ...
  writeUint16(val: number) {
    this.ensureCapacity(2);
    const view = new DataView(this.buffer.buffer, this.buffer.byteOffset, this.buffer.byteLength);
    view.setUint16(this.offset, val, true);
    this.offset += 2;
  }
  ...
  writeInt32(val: number) {
    ...
    const view = new DataView(this.buffer.buffer, this.buffer.byteOffset, this.buffer.byteLength);
    view.setInt32(this.offset, val, true);
    this.offset += 4;
  }
  ...
  writeUint32(val: number) {
    this.ensureCapacity(4);
    const view = new DataView(this.buffer.buffer, this.buffer.byteOffset, this.buffer.byteLength);
    view.setUint32(this.offset, val, true);
    this.offset += 4;
  }
  ...
  writeFloat32(val: number) {
    this.ensureCapacity(4);
    const view = new DataView(this.buffer.buffer, this.buffer.byteOffset, this.buffer.byteLength);
    view.setFloat32(this.offset, val, true);
    this.offset += 4;
  }
  ...
  writeFloat64(val: number) {
    ...
    const view = new DataView(this.buffer.buffer, this.buffer.byteOffset, this.buffer.byteLength);
    view.setFloat64(this.offset, val, true);
    this.offset += 8;
  }
...
function serializeStructuredData(id: number, value: FLPEvent['value']): Uint8Array {
  ...
  switch (id) {
    ...
    default:
      // Fallback: if it's some unrecognized event but holds Uint8Array, we already handled it.
      // Otherwise, write nothing or throw
